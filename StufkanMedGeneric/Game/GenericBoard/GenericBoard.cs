...
{
    public partial class GenericBoard<T> : UserControl
    {
        /// <summary>
        ...
        public GenericBoard(T[,] grid, Color background)
            : this(grid)
        {
        ...
        public GenericBoard(T[,] grid, Color background, T[,] bgGrid)
        ...
        public void SetCell(Point cell, T piece)
        {
            grid[cell.X, cell.Y] = piece;
        }

        /// <summary>
        /// Removes the T from the cell
        /// </summary>
        /// <param name="cell"></param>
        public void ResetSetCell(Point cell)
        {
            grid[cell.X, cell.Y] = default(T);
        }
        ...
        public void DrawGrid(Graphics gfx, T[,] grid, SolidBrush bgBrush, int pensize)
        {
                ...
                for (int j = 0; j < rows; j++)
                {
                    if (grid[j, i] == null)
                    {
                        if (bgBrush.Color.Name != "0")
                            gfx.FillRectangle(bgBrush, x + pensize, y + pensize, rectSize - pensize, rectSize - pensize);
                    }
                    else
                    {
                        gfx.DrawImage((grid[j, i] as Piece).Model, x + pensize, y + pensize, rectSize - pensize, rectSize - pensize);
                    }
                    x += rectSize;
        ...
        public void DrawBackground(Graphics gfx, int pensize)
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < rows; j++)
                {
                    gfx.FillRectangle(new SolidBrush(Background), x + pensize, y + pensize, rectSize - pensize, rectSize - pensize);
                    x += rectSize;
                }
                x = 0;
                y += rectSize;
        ...
        public void changePattern(BoardPattern<T> pattern)
        {
